...
log files and generates Single Point Energy (SPE) input files.
...
Output:
    One .gjf file per log file, e.g. 200.log -> 200_spe.gjf
...
MEM           = "48GB"
NPROC         = 8
CHK_SUFFIX    = "_spe"   # appended before .chk  →  {base}{CHK_SUFFIX}.chk
                          # set to "" for plain {base}.chk
GJF_SUFFIX    = "_MC"    # appended before .gjf  →  {base}{GJF_SUFFIX}.gjf
# ─────────────────────────────────────────────

# Atomic number → element symbol map (extend if needed)
...
# Basis/ECP sections — matches the original calculation:
#   6-311G* for all atoms except Fe; SDD ECP for Fe
BASIS_SECTION = """\
H N C B 0
6-311G*
****
...
def parse_last_input_orientation(log_path):
    """
    Returns a list of (element_symbol, x, y, z) tuples from
    the LAST 'Input orientation:' block in the log file.
    """
    atoms = []
    in_block = False
    header_lines = 0
    ...
    with open(log_path, "r") as f:
        for line in f:
            if "Input orientation:" in line:
                # Start fresh for each new block found
                atoms = []
                in_block = True
                header_lines = 0
                continue
            ...
            if in_block:
                # Skip the 4 header/separator lines
                if header_lines < 4:
                    header_lines += 1
                    continue
                # Blank or separator line signals end of block
                stripped = line.strip()
...
def get_charge_from_log(log_path):
    """Reads charge from 'Charge = X Multiplicity = Y' line."""
    with open(log_path, "r") as f:
        for line in f:
            if "Charge =" in line and "Multiplicity =" in line:
                ...
                try:
                    ...
                    pass
    return CHARGE  # fallback to user setting


def write_spe_input(atoms, log_path, charge):
    """Writes the Gaussian SPE .gjf file."""
...
if __name__ == "__main__":
    # Collect targets from command-line args or glob *.log in current dir
    targets = []
    if len(sys.argv) > 1:
