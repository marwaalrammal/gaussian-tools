Marwa Al Rammal May 15 2026
#!/usr/bin/env python3
"""
track_root.py
=============
Reads a Gaussian TD-DFT geometry-optimization log and monitors whether
the tracked excited state (root N) keeps its character throughout, or
whether a root flip has occurred.

For every optimization step it:
  - Reports energy (eV), oscillator strength (f), spin contamination <S²>
  - Shows the dominant orbital transition
  - Computes cosine similarity of the current step's state fingerprint
    against the PREVIOUS step's fingerprint for ALL states, so it can
    detect when another state has 'become' the target state.
...
Output
------
  - Step-by-step table printed to the terminal
  - FLIP warning when another state matches the previous target better
  - Final recommendation: CONTINUE / RESTART
"""
